            ...
            <div className="space-y-2 mb-8">
              {today.tasks.map((task) => (
                <div key={task.id} className="flex items-center gap-4 p-5 rounded-2xl hover:bg-white/60 transition-all group cursor-pointer border border-transparent hover:border-rose-pastel/20"
                  ...
                  <span className={`flex-1 text-sm ${task.completed ? 'line-through text-soft-text/20' : 'text-soft-text font-bold'}`}>{task.text}</span>
                </div>
              ))}
            </div>
            <div className="flex gap-3 bg-white/60 backdrop-blur-md p-2 rounded-2xl border border-rose-pastel/40 shadow-sm">
              <input type="text" value={newTask} onChange={(e) => setNewTask(e.target.value)} onKeyPress={(e) => e.key === 'Enter' && addTask()} placeholder="¿Qué más haremos hoy?" className="flex-1 bg-transparent px-4 py-3 outline-none text-soft-text text-sm font-bold" />
              <button onClick={addTask} className="bg-deep-rose text-white p-3 rounded-xl shadow-lg shadow-pink-200 active:scale-95 transition-all"><Plus size={20} /></button>
            </div>
          </section>
        </div>

        <div className="lg:col-span-5">
          <section className="bg-white/40 backdrop-blur-xl p-10 rounded-[3.5rem] border border-white/60 journal-shadow lg:sticky lg:top-12">
