        ...
        <div className="flex items-center justify-between gap-4">
          <div className="space-y-2 min-w-0">
            ...
            <div className="flex items-center gap-3 flex-wrap">
              <p className="text-theme-primary font-semibold text-xs sm:text-sm lg:text-base tracking-wide opacity-80">
          ...
          {today.mood && (
            <motion.div initial={{ scale: 0.8 }} animate={{ scale: 1 }} className="bg-white/60 backdrop-blur-md p-3 sm:p-4 rounded-[2rem] border border-theme-border flex items-center gap-3 shadow-sm shrink-0">
              ...
              <button onClick={() => updateToday({ mood: null })} className="text-theme-primary/40 hover:text-theme-primary text-[10px] underline uppercase font-bold">Cambiar</button>
            </motion.div>
          )}
        </div>

        {!today.mood && (
          <div className="bg-white/40 backdrop-blur-sm p-4 rounded-[1.5rem] border border-theme-border/50 shadow-sm max-w-md mx-auto sm:mx-0">
            ...
            <div className="grid grid-cols-2 gap-2">
              {MOODS.map((m) => (
                <button key={m.label} onClick={() => updateToday({ mood: m.label })} className="flex items-center gap-3 px-4 py-3 rounded-xl bg-white/60 border border-theme-border/30 hover:border-theme-primary/40 hover:bg-white/80 transition-all active:scale-95">
            ...
            <div className="space-y-4">
              {today.priorities.map((p, i) => (
                <input
                  ...
                  className="w-full bg-white/40 border border-theme-border/20 rounded-2xl px-6 py-5 text-soft-text font-bold outline-none focus:bg-white focus:border-theme-primary/20 shadow-inner-sm transition-all"
                />
              ))}
            </div>
          </section>
          ...
          <section className="card-premium">
            ...
            <div className="space-y-2 mb-8">
              {today.tasks.map((task) => (
                <div key={task.id} className="flex items-center gap-4 p-5 rounded-2xl hover:bg-white/60 transition-all group cursor-pointer border border-transparent hover:border-theme-border/20"
                  ...
                  <div className={`transition-all ${task.completed ? 'scale-90 opacity-40' : 'group-hover:scale-110'}`}>
                    {task.completed ? <CheckCircle2 size={24} fill="var(--color-theme-accent)" className="text-white" /> : <Circle size={24} className="text-theme-pastel" />}
                  </div>
            ...
            <div className="flex gap-3 bg-white/60 backdrop-blur-md p-2 rounded-2xl border border-theme-border/40 shadow-sm">
              ...
              <button onClick={addTask} className="bg-theme-primary text-white p-3 rounded-xl shadow-lg active:scale-95 transition-all shrink-0"><Plus size={20} /></button>
            </div>
          </section>
        </div>
        ...
        <div className="lg:col-span-5">
          <section className="bg-white/40 backdrop-blur-xl p-8 md:p-10 rounded-[3.5rem] border border-white/60 journal-shadow lg:sticky lg:top-12">
            <div className="flex flex-col items-center mb-10">
              <div className="w-16 h-16 bg-white rounded-full flex items-center justify-center shadow-sm mb-6 border border-theme-border/20">
                <Heart size={24} fill="var(--color-theme-accent)" className="text-theme-primary" />
              </div>
              <h3 className="text-[10px] font-black uppercase tracking-[0.3em] text-theme-primary/60">Cosecha de Gratitud</h3>
            </div>
            <div className="space-y-6 text-center">
              {today.gratitude.map((g, i) => (
                <textarea
                  ...
                  className="w-full bg-white/60 backdrop-blur-sm border border-theme-border/30 rounded-[2rem] p-6 outline-none focus:bg-white focus:border-theme-primary/20 transition-all text-soft-text text-sm italic font-medium resize-none shadow-sm"
                />
              ))}
            </div>
          </section>
